...
from multiprocessing import Process, Array, RLock, Semaphore, Manager, cpu_count
import sys
...
# def factorize(valie: Array, number):
#     with valie.get_lock():
#         out_list = []
#         for i in range(1, number + 1):
#             if number % i == 0:
...
# def factorize_asinc_mode(*number) -> list:
#     value=Array('i', size_or_initializer=1, lock=RLock())
#     process = []
#     for item in number:
#         pr = Process(target=factorize, args=(value, item,))
#         pr.start()
#         process.append(pr)
#     [pr.join() for pr in process]
